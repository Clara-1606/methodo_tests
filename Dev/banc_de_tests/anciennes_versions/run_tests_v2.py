...
def generate_html_report(timestamp, timestamp_dir, total_tests, tests_ok, success_rate, test_results):
    ...
    <head>
        ...
        <style>
            ...
            .diff-table .self-column {{ color: green; }}
        </style>
    </head>
    <body>
    ...
    for test_name, result, stdout, stderr,differences in test_results:
        if not result :
            ...
            <li class="failure">
                ...
                <pre>{stderr}</pre>
                <h3>Différences :</h3>
                {differences_to_html(differences)}
            </li>
            """
        else : 
             html_template += f"""
...
def differences_to_html(differences):
    if differences is None:
        return ""

    html = differences.to_html(classes='diff-table', index=False, na_rep='', justify='left', escape=False)

    # Remplacer 'self' par une classe CSS verte pour df1 et une classe CSS rouge pour df2
    html = html.replace('<td>self</td>', '<td class="df1-column">self</td>')
    html = html.replace('<th>self</th>', '<th class="df1-column">self</th>')
    html = html.replace('<td>other</td>', '<td class="df2-column">other</td>')
    html = html.replace('<th>other</th>', '<th class="df2-column">other</th>')

    # Parcourir chaque ligne pour appliquer la couleur en fonction des valeurs
    lines = html.split('\n')
    for i in range(1, len(lines) - 1):  # Commence à 1 pour éviter l'entête, finit à len(lines) - 1 pour éviter la dernière balise </table>
        cols = lines[i].split('</td>')
        for j in range(1, len(cols) - 1):  # Commence à 1 pour éviter la première balise <tr> et finit à len(cols) - 1 pour éviter la dernière balise <td>
            if 'df1-column' in cols[j - 1]:  # Vérifie la colonne précédente pour 'df1'
                if 'df2-column' in cols[j]:  # Vérifie la colonne actuelle pour 'df2'
                    cols[j] = cols[j].replace('<td class="df2-column">', '<td class="df2-column" style="color:red;">')
            elif 'df2-column' in cols[j - 1]:  # Vérifie la colonne précédente pour 'df2'
                if 'df1-column' in cols[j]:  # Vérifie la colonne actuelle pour 'df1'
                    cols[j] = cols[j].replace('<td class="df1-column">', '<td class="df1-column" style="color:green;">')

        lines[i] = '</td>'.join(cols)
